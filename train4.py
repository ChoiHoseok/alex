...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        c2, _, summary2, accuracy2 = m2.train(batch_x,batch_y,merged)
        avg_cost += c / total_batch
        avg_cost2 += c2 / total_batch

    print('[m1] Epoch:', '%04d' % (epoch + 1), 'cost =', '{:.9f}'.format(avg_cost))
    print('Accuracy:', accuracy)
    print('[m2] Epoch:', '%04d' % (epoch + 1), 'cost =', '{:.9f}'.format(avg_cost2))
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        c2, _, summary2, accuracy2 = m2.train(batch_convert_x, batch_convert_y, merged)
        avg_cost += c / total_batch
        avg_cost += c2 / total_batch

    print('[m1] Epoch:', '%04d' % (training_epochs + epoch + 1),
         'cost =', '{:.9f}'.format(avg_cost))
    print('[m1] Accuracy:', accuracy)
    print('[m2] Epoch:', '%04d' % (training_epochs + epoch + 1),
         'cost =', '{:.9f}'.format(avg_cost))
    print('[m2] Accuracy:', accuracy2)
train_writer.add_summary(summary)
...
correct_prediction = tf.equal(
            tf.argmax(total_pd, 1), tf.argmax(Y_t, 1))
